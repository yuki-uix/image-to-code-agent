...
if (!args.analysis) {
  ...
  const result = await architect.extractWithTrace(visualAnalysis);
  ...
  const report = validateComponentRegistry(result.registry, visualAnalysis);
  ...
  await writeJson(join(outputDir, "run.json"), {
    ...
    artifacts: ["component-registry.json", "raw-component-registry.json", "raw-component-registry.txt", "component-validation.json", "run.json"]
  });
